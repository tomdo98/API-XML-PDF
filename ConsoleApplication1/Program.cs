...
using common;
    ...
    class firegetAPInvoices
    {
        ...
        public static void tax_rules_expense(string ConString)
        {
            string connectionString = null;
            ...
            SqlConnection cnn2;
            //connectionString2 = "Server=FINSYS-DB-TST\\FINSYSTST;Database=WFM_Basware_Prod;Trusted_Connection=False;User ID=bwadmin;Password =bw@dm1ntest";
            connectionString2 = ConString;
            ...
            decimal iSubtotal; 
            decimal iTotal; 
            decimal d_TaxPctThreshold; 
            decimal iTax; 
            decimal iGST; 
            decimal iPST; 
            decimal iTaxRate;
            decimal iTax_InvSub;
            string tax_reporting="";
            decimal iTaxRate_Sub1;
            decimal iTaxRate_Sub2;
            decimal iTaxRate_Sub3;
            decimal iTaxRate_Sub4;
            string guidsesh;
            string guiddoc;
            ...
            sql = "select subtotal, installation, freight, tax_total, ps_tax_rate, gross_invoice_amount, threshhold_pct, session_guid, document_guid from prestage_invoice_esb where tax_reporting = '' and document_class='Expense' and pdf_created = 'Y'";
            try
            {
                cnn.Open();
                ...
                while (dataReader.Read())
                {
                    iSubtotal = dataReader.GetDecimal(0);
                    iInstall = dataReader.GetDecimal(1);
                    iFreight = dataReader.GetDecimal(2);
                    iTax = dataReader.GetDecimal(3);
                    iTaxRate = dataReader.GetDecimal(4);
                    iTotal = dataReader.GetDecimal(5);
                    d_TaxPctThreshold = dataReader.GetDecimal(6);
                    guidsesh = dataReader.GetString(7);
                    guiddoc = dataReader.GetString(8);

                       //'*If the subtotal was not extracted during OCR, calculate it for tax evaluation.
                       if (iSubtotal == 0)
                       {
                          iTax_InvSub = iTotal - iTax - iFreight;
                       }
                       else
                       {
                          iTax_InvSub = iSubtotal;
                       }

                       //'*Set the subtotal tax rates. iSub1 = Cost of Goods.  iSub2 = Cost of Goods + Freight.
                       if (iTax_InvSub != 0)
                       {
                          iTaxRate_Sub1 = (iTax/iTax_InvSub)*100;
                       }
                       else
                       {
                          iTaxRate_Sub1 = 0;
                       ...
                       if ((iTax_InvSub + iFreight) != 0)
                       {
                          iTaxRate_Sub2 = (iTax / (iTax_InvSub + iFreight))*100;
                       }
                       else
                       {
                          iTaxRate_Sub2 = 0;
                    ...
                    //‘iTaxRate is the tax rate from PeopleSoft Ship to ID

                      //'*Sub1 and Sub2 = Tax or Freight, no Installation (original tax scenario)
                          if (Math.Abs(iTax_InvSub) > 0.01m) 
                          {
                             if ((iTaxRate_Sub1 == iTaxRate) || (Math.Abs(iTaxRate_Sub1 - iTaxRate) < d_TaxPctThreshold))
                              {
                                 tax_reporting  = "Freight Not Taxed";
                              }
                              else 
                              {
                                  if ((iTaxRate_Sub1 > iTaxRate) & (iTaxRate_Sub2 > iTaxRate)) 
                                  {
                                    tax_reporting= "Tax Too High";
                                  }
                                  else
                                  {
                                      if ((iTaxRate_Sub1 > iTaxRate) & (Math.Abs(iTaxRate_Sub2 - iTaxRate) < d_TaxPctThreshold)) 
                                      {
                                        tax_reporting= "Freight Taxed";
                                      }
                                      else
                                      {
                                          if ((iTaxRate_Sub1 > iTaxRate) & (iTaxRate_Sub2 < iTaxRate)) 
                                          {
                                            tax_reporting= "TW Review";
                                          }
                                          else
                                          {
                                            tax_reporting = "Tax Too Low";
                                          }
                                      }
                                  }
                              }
                          }
                          else //'//*Only charge on invoice is Freight
                          {
                             if (Math.Abs(iTaxRate_Sub2 - iTaxRate) < d_TaxPctThreshold) 
                             {
                                   tax_reporting= "Freight Taxed";
                             }
                             else
                             {
                                 if  (iTaxRate_Sub2 > iTaxRate) 
                                 {
                                   tax_reporting= "Tax Too High";
                                 }
                                 else
        ...
        public static void tax_rules_construction(string ConString)
        {
            string connectionString = null;
            //SqlConnection connection; 
            SqlCommand command;
            ...
            connectionString = ConString;
            cnn = new SqlConnection(connectionString);
            sql = "select subtotal, installation, freight, tax_total, ps_tax_rate, gross_invoice_amount, threshhold_pct, session_guid, document_guid from prestage_invoice_esb where tax_reporting = '' and document_class='Construction' and pdf_created = 'Y'";
            try
            {
                cnn.Open();
                command = new SqlCommand(sql, cnn);
                ...
                while (dataReader.Read())
                {
                    ...
                    if (iSubtotal == 0) {
                        iTax_InvSub = iTotal - iTax - iFreight - iInstall;}
                    else
                    {    iTax_InvSub = iSubtotal;}
                   ...
                   if ((iTax_InvSub + iInstall) != 0) {
                      iTaxRate_Sub3 = (iTax / (iTax_InvSub + iInstall))*100;}
                   else
                    {   iTaxRate_Sub3 = 0;}

                   if ((iTax_InvSub + iInstall + iFreight) != 0) {
                      iTaxRate_Sub4 = (iTax / (iTax_InvSub + iInstall + iFreight))*100;}
                   else
                    {   iTaxRate_Sub4 = 0;}
                   ...
                   if (iInstall == 0)
                   {
                    ...
                    if (((iTaxRate_Sub1 == iTaxRate) || (Math.Abs(iTaxRate_Sub1 - iTaxRate) < d_TaxPctThreshold)) & iFreight == 0.00m)
                   {
                        tax_reporting = "";
                   }

//sub3 scenario
                    //  '*Sub3 Scenario = Installation and Tax, no Freight
                    if (iInstall > 0 & iFreight == 0)
                    {
                        if (Math.Abs(iTax_InvSub) > 0.01m)
                        {
                            if ((iTaxRate_Sub1 == iTaxRate) || (Math.Abs(iTaxRate_Sub1 - iTaxRate) < d_TaxPctThreshold))
                            {
                                tax_reporting = "Install Not Taxed";
                            }
                            else
                            {
                                if (Math.Abs(iTaxRate_Sub3 - iTaxRate) < d_TaxPctThreshold)
                                {
                                    tax_reporting = "Install Taxed";
                                }
                                else
                                {
                                    if (iTaxRate_Sub1 < iTaxRate)
                                    {
                                        tax_reporting = "Tax Too Low";
                                    }
                                    else
                                    {
                                        if (iTaxRate_Sub3 > iTaxRate)
                                        {
                                            tax_reporting = "Tax Too High";
                                        }
                                        else
                       ...
                       //*Sub4 Scenario = Installation and Tax and Freight
                      if (iInstall > 0 & iFreight > 0)
                      {
                          if (Math.Abs(iTax_InvSub) > 0.01m)
                          {
                              ...
                              else
                              {
                                  if ((iTaxRate_Sub1 == iTaxRate) || ((Math.Abs(iTaxRate_Sub1) - iTaxRate) < d_TaxPctThreshold))
                                  {
                                      tax_reporting = "Freight and Install Not Taxed";
                                  }
                                  else
                                  {
                                      if (Math.Abs(iTaxRate_Sub2 - iTaxRate) < d_TaxPctThreshold)
                                      ...
                                      else
                                      {
                                          ...
                                          {
                                              if (Math.Abs(iTaxRate_Sub4 - iTaxRate) < d_TaxPctThreshold)
                                              {
                                                  tax_reporting = "Freight and Install Taxed";
                                              }
                                              else
                                              {
                                                  if (iTaxRate_Sub4 > iTaxRate)
                                                  {
                                                      tax_reporting = "Tax Too High";
                                                  }
                                                  else
                          ...
                          {
                              //*Only charge on invoice is Installation and Freight
                              if (Math.Abs(iTaxRate_Sub2 - iTaxRate) < d_TaxPctThreshold)
                              {
                                  tax_reporting = "Install Not Taxed";
                              }
